...
static int S_render_node(cmark_node *node, cmark_event_type ev_type,
  ...
  if (entering) {
    ...
    cmark_strbuf_puts(xml, buffer);
    snprintf(buffer, BUFFER_SIZE, " extents=\"%d:%d\"",
             node->extents.start, node->extents.stop);
    cmark_strbuf_puts(xml, buffer);
    snprintf(buffer, BUFFER_SIZE, " end-extents=\"%d:%d\"",
             node->end_offsets.start, node->end_offsets.stop);
    cmark_strbuf_puts(xml, buffer);
