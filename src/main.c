...
static void print_extensions(void) {
  ...
  block_parsers = cmark_list_block_parsers();
  for (tmp = block_parsers; tmp; tmp=tmp->next) {
    cmark_block_parser *ext = (cmark_block_parser *) tmp->data;
    printf("%s\n", ext->name);
  }

  cmark_llist_free(block_parsers);
}
...
int main(int argc, char *argv[]) {
  ...
  for (i = 1; i < argc; i++) {
    if ((strcmp(argv[i], "-e") == 0) || (strcmp(argv[i], "--extension") == 0)) {
      i += 1;
      if (i < argc) {
        cmark_block_parser *block_parser = cmark_find_block_parser(argv[i]);
        if (!block_parser) {
          fprintf(stderr, "Unknown extension %s\n", argv[i]);
          goto failure;
        }
        cmark_parser_attach_block_parser(parser, block_parser);
      } else {
        fprintf(stderr, "No argument provided for %s\n", argv[i - 1]);
        goto failure;
      }
